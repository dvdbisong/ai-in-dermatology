...
def set_directory(image_dir: str):
    """set image directory"""
    global directory
    directory = image_dir
...
class dataGenerator(object):
    ...
    def __init__(self, loc, flip=True, suffix='png'):
        ...
        try:
            os.mkdir("data-npy/" + loc + "-npy-" + str(im_size))
        except:
        ...
        np.save("data-npy/" + loc + "-npy-" + str(im_size) + "/data.npy", self.files)
    ...
    def load_from_npy(self, loc):
        ...
        self.files = np.load("data-npy/" + str(loc) + "-npy-" + str(im_size) + "/data.npy")
...
class BiGAN(object):
    ...
    def evaluate(self, num=0):
        ...
        x.save("results/{}/i".format(directory) + str(num) + ".png")
        ...
        x.save("results/{}/i".format(directory) + str(num) + "-ema.png")

    def prepareSamples(self, cnum=0, num=1000):  # 8x8 images, bottom row is constant

        try:
            os.mkdir("results/{}/Samples-c".format(directory) + str(cnum))
        except:
            x = 0
        ...
        for i in range(im.shape[0]):
            ...
            x.save("results/{}/Samples-c".format(directory) + str(cnum) + "/im (" + str(i + 1) + ").png")
    ...
    def saveModel(self, model, name, num):
        ...
        model.save_weights("models/{}/".format(directory) + name + "_" + str(num) + ".h5")

    def loadModel(self, name, num):

        file = open("models/{}/".format(directory) + name + ".json", 'r')
        json = file.read()
        file.close()

        mod = model_from_json(json)
        mod.load_weights("models/{}/".format(directory) + name + "_" + str(num) + ".h5")
...
def main(steps: int):
    ...
    while model.GAN.steps <= steps:  # 600000:
        model.train()
